...
use nix::errno::Errno;
use nix::libc::{
    epoll_create, epoll_ctl, epoll_event, epoll_wait, setsockopt, EPOLLERR, EPOLLHUP, EPOLLIN, EPOLLPRI, EPOLLRDHUP, EPOLL_CTL_ADD, IPPROTO_IP, IPTOS_ECN_CE, IP_TOS
};
...
fn accurate_ecn_tcp(
    ...
    unsafe {
        let result = setsockopt(
            ...
            IPPROTO_IP,
            IP_TOS,
            &IPTOS_ECN_CE as *const u8 as *const std::ffi::c_void,
            1,
        );
        info!(logger, "Result of setsockopt: {result}");
    }
    ...
    for (targets_complete, (rank, target)) in potential_targets.iter().enumerate() {
        ...
        for target in target.into_iter() {
            if let FlexibleIp::Ipv4(_) = target {
                ...
                //accurate_ecn_udp(&target.clone(), &canonical_target, logger.clone());
                let quic_result =
